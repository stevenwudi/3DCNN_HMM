...
from Main_3DCNN import ConvNet
...
#        pl.ylabel(self.show_cost)
    ...
    def plot_filters(self):
        ...
        if layer['type'] == 'fc': # Fully-connected layer
            ...
            #if layer['type'] == 'local':
            #    filters = filters.reshape((channels*layer['filterPixels'][self.input_idx],layer['modules'], num_filters))
            #    filters = filters.reshape((channels*layer['filterPixels'][self.input_idx], num_filters*layer['modules']))
            #    #filters = filters.swapaxes(0,1).reshape(channels * layer['filterPixels'][self.input_idx], num_filters * layer['modules'])
            #    num_filters *= layer['modules']

        filters = filters.reshape(channels, filters.shape[0]/channels, filters.shape[1])
        #filters = filters.swapaxes(0,1)
        ...
        #filters = filters[:,:,3000:3064]
        #num_filters = 64
        # Make sure you don't modify the backing array itself here -- so no -= or /=
        filters = filters - filters.min()
        filters = filters / filters.max()

        #f_min = filters.min(axis=0)
        #f_min = f_min.min(axis=0)
        #filters = filters - f_min
        #f_max = filters.max(axis=0)
        #f_max = f_max.max(axis=0)
    ...
    def do_write_features(self, data):
        self.ftr_layer_idx = self.get_layer_idx('probs')
        num_ftrs = self.layers[self.ftr_layer_idx]['outputs']
        ftrs = n.zeros((data.shape[1], num_ftrs), dtype=n.single)
        labels_temp = n.zeros((1,data.shape[1]), dtype=n.single)
        self.libmodel.startFeatureWriter([data, labels_temp]+ [ftrs], self.ftr_layer_idx)           
        self.finish_batch()
        return ftrs
